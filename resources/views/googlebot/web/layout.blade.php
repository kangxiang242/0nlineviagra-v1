    ...
    <nav class="nav-sec">
        ...
        <ul class="base">
            <li class="link-parent" tabindex="0">
                <h2 class="base-link">威而鋼 VIAGRA<sup>®</sup>&nbsp;線上訂購<i class="iconfont">&#xeca2;</i></h2>
                <ul class="mega-menu">
                    @foreach($categorys as $item)
                    <li class="link-item"><a href="{{ $item->uri?url($item->uri):"javascript:;" }}">威而鋼 VIAGRA<sup>®</sup>&nbsp;{{ $item->name }}</a></li>
                    @endforeach
                </ul>
            </li>
        ...
        <aside class="ad-carousel swiper-container">
            <div class="swiper-wrapper">
                ...
                <article class="swiper-slide" itemscope itemtype="https://schema.org/Product">
                    <img class="ad-pic" itemprop="image" src="{{ storage_url($setting->get('ad_img')) }}" alt="{{ $item->describe }} 威而鋼VIAGRA® {{ $item->name }} 產品圖片展示">
                    <div class="ad-text">
                        <h2 class="p1" itemprop="name"><span>威而鋼 VIAGRA<sup>®</sup> {{ $item->name }}</span>{{ $item->describe_gb }}</h2>
        ...
        <header class="page-header {{ $class }}">
            ...
            @if($banner)
                <picture class="banner">
                    <source media="(max-width: 1024px)" srcset="{{ storage_url($banner->img) }}">
                    <source media="(min-width: 1025px)" srcset="{{ storage_url($banner->img) }}">
                    <img src="{{ storage_url($banner->img) }}" alt="響應式 Banner">
                </picture>
                @yield('embed-banner')
            @endif
        </header>
...
<footer class="main-footer">
    ...
    <nav class="main-links">
        <h2 class="links-title">輝瑞威而鋼 VIAGRA<sup>®</sup>&nbsp;線上訂購：</h2>
        <ul class="links-list">
            @foreach($categorys as $item)
                <li class="links-item"><a href="{{ $item->uri?url($item->uri):"javascript:;" }}">威而鋼 VIAGRA<sup>®</sup>&nbsp;{{ $item->name }}</a></li>
            @endforeach
        </ul>
    </nav>

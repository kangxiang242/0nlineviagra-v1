...
if(flash_data){
    ...
    var _flash = flash_data;

    if(_flash.code==200){
        xie.success(_flash.msg,_flash.sub_msg);
    }else if(_flash.code==400){
        xie.error(_flash.msg,_flash.sub_msg);
        autoFillFormData(_flash.form_data)
    }
}

function autoFillFormData(form_data){
    for (const key in form_data) {
        $("input[name='"+key+"']").val(form_data[key]);
        $("textarea[name='"+key+"']").val(form_data[key]);
